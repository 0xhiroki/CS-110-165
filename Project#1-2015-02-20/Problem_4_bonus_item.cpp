/***************************************************************
Problem: Project #1 Problem#4 bonus
...
(without using if/else statement, switch, or ternery operator)
Name: Hiroki Hori
ID: 1496802
Date: 02/20/2015
Status: complete
...
int main() {
    ...
    tax = static_cast<int>(income > TEN_PERCENT_TAX_MINIMUM) * ((income - TEN_PERCENT_TAX_MINIMUM) * TEN_PERCENT_TAX + (TEN_PERCENT_TAX_MINIMUM - NO_TAX_MAXIMUM) * FIVE_PERCENT_TAX) + static_cast<int>(income <= TEN_PERCENT_TAX_MINIMUM) * ((income - NO_TAX_MAXIMUM) * FIVE_PERCENT_TAX);
